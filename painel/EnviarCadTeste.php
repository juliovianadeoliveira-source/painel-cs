...
$cupom = (isset($_POST['cupom'])) ? trim($_POST['cupom']) : '';
	...
	elseif(empty($EditarCelular)){
		echo MensagemAlerta($_TRA['erro'], "CELULAR E CAMPO OBRIGATORIO", "danger");
	}
	elseif( substr_count($email, "@") == 0 || substr_count($email, ".") == 0) {
		echo MensagemAlerta($_TRA['erro'], $_TRA['dei'], "danger");
	}
	elseif( substr_count($email, "gmail") == 0 ){

		echo MensagemAlerta('	aviso', 'COMUNICADO IMPORTANTE <br/>Não será permitido e-mails temporários. IPs e e-mails que forem gerado mais de duas vezes também será excluído. <br/>
			Desde ja nossa equipe agradece sua atenção.<br/>
			Permitido somente e-mails:<b> GMAIL</b>', "warning");
	}
